...
flist = sorted(list(glob.glob('%sa.pm*dec.nc' % runid)))
if not flist:
    print("Nothing to process")
    sys.exit(0)

lastfile = flist[-1]
lastfile_year = int(lastfile[9:13])
...
if lastfile_year > lastyear:
    ...
    for year in range(lastyear+1, lastfile_year+1):
        ...
        flist_year = glob.glob('%sa.pm%4.4d???.nc' % (runid, year))
        ...
        for mon in range(12):
            fname = '%sa.pm%4.4d%s.nc' % (runid, year, mname[mon])
            print(fname)
